"""Experimental via absorption and continuation; static copy of production.

Only the diagnostic context manager activates this module. No KRT changes.
"""
from contextlib import contextmanager
from unittest.mock import patch
from collections import deque
from dgloss.krt_api import point_to_pad_distance, point_to_segment_distance, SpatialIndex, via_copper_layers
from dgloss.pad_terminals import _pad_on_layer


def _at_pad(context, via):
    # Pads are immutable during Gloss. Reuse KRT's rotation-aware pad index.
    # Cell size bounds the query radius; it is not a routing/clearance grid.
    index = getattr(context, '_progressive_pad_index', None)
    if index is None:
        pcb = context.pcb_data
        index = SpatialIndex(cell_size=max(1., max((v.size/2+1e-6 for v in pcb.vias), default=0.)))
        layers = pcb.board_info.copper_layers
        for net, pads in pcb.pads_by_net.items():
            for pad in pads:
                index.add_pad(pad, net, [layer for layer in layers if _pad_on_layer(pad, layer)])
        context._progressive_pad_index = index
    return any(net == via.net_id and point_to_pad_distance(via.x, via.y, pad) <= via.size/2 + 1e-6
               for layer in via_copper_layers(via, context.pcb_data.board_info.copper_layers)
               for pad, net in index.get_nearby_pads(via.x, via.y, layer))


def _progressing_vias(context, net):
    pcb = context.pcb_data
    queue = deque(v for v in pcb.vias if v.net_id == net)
    while queue:
        old = queue.popleft()
        if _at_pad(context, old):
            continue
        before_vias, before_segments = pcb.vias, pcb.segments
        yield old
        if pcb.vias is before_vias:
            continue
        before = {id(v) for v in before_vias}
        anchor_positions = {_other_end(s, pos_key(old.x, old.y)) for s in before_segments
                            if s.net_id == net and pos_key(old.x, old.y) in
                            (pos_key(s.start_x, s.start_y), pos_key(s.end_x, s.end_y))}
        added = [v for v in pcb.vias if id(v) not in before and v.net_id == net]
        if len(added) != 1 or (added[0].x, added[0].y) not in anchor_positions:
            continue
        moved = added[0]
        if _at_pad(context, moved):
            continue
        # A T/interior crossing, another via or locked copper is a fixed stop.
        touching = [s for s in pcb.segments if s.net_id == net
                    and s.layer in via_copper_layers(moved, pcb.board_info.copper_layers) and point_to_segment_distance(
                        moved.x, moved.y, s.start_x, s.start_y, s.end_x, s.end_y) <= 1e-7]
        if (len(touching) != 2 or touching[0].layer == touching[1].layer
                or any(getattr(s, 'locked', False) or getattr(s, 'graphic', False) for s in touching)
                or any(v is not moved and pos_key(v.x,v.y)==pos_key(moved.x,moved.y) for v in pcb.vias)):
            continue
...
from dgloss.topology import check_local_connectivity as check_net_connectivity
from dgloss.krt_api import Segment
from dgloss.krt_api import calculate_route_length
from dgloss.krt_api import pos_key

from dgloss.algorithm import (_clears_krt_grid, _connectivity_worse, _right_angle,
                        _touches_other_same_net)
from dgloss.changes import GlossChanges, release_result_custody
...
def move_mobile_vias(context, results, *, net_ids, stage="G3.1",
    ...
    stage_segments = {id(s) for s in context.pcb_data.segments}
    stage_vias = {id(v) for v in context.pcb_data.vias}
    ...
    for net_id in net_ids:
        ...
        for old_via in _progressing_vias(context, net_id):
            ...
            if full_chains:
                ...
                # simple-chain walker.
                from dgloss.sliding_nodes import _walk_branch_chain
            ...
            for position in _candidate_positions(
                ...
                if old_length - new_length <= context.coord.grid_step + 1e-12:
                    continue
                score = (new_length, len(candidate), math.hypot(position[0]-old_via.x, position[1]-old_via.y), position[0], position[1])
    ...
    # Discard copper created then absorbed before this stage is exported.
    live_segments = {id(s) for s in context.pcb_data.segments}
    live_vias = {id(v) for v in context.pcb_data.vias}
    input_vias = [v for v in input_vias if id(v) in stage_vias]
    emitted_vias = [v for v in emitted_vias if id(v) in live_vias]
    segment_strips = [s for s in segment_strips if id(s) in stage_segments]
    added_segments = [s for s in added_segments if id(s) in live_segments]
    changes.segments = [c for c in changes.segments
                       if ('old' in c and id(c['old']) in stage_segments)
                       or ('new' in c and id(c['new']) in live_segments)]
...
def progressive_via():
    ...
    with patch.object(pipeline, 'move_mobile_vias', move_mobile_vias), \
            patch.object(pipeline, 'refine_mobile_vias', refine_mobile_vias):
        yield
